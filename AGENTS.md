...
- 2026-06-13: `AppShell` must wait for auth-store hydration before calling `refreshProfile()` / `file-store.bootstrap()`. `auth-store.refreshProfile()` should also reset stale auth/file state on failure instead of leaving half-initialized session data around.
- 2026-06-13: Preview security comments must not overclaim protections. DOCX zip validation is fine; do not state that stdlib XML monkeypatching hardens `python-docx`/`lxml` unless that is demonstrably true.
- 2026-06-13: Billing MVP may switch plans server-side without payment flow. `auth-service` owns source of truth via user `storage_quota`; frontend should call auth endpoint, update local user/quota state immediately, and not wait for file-service quota cache expiry.
- 2026-06-13: After plan switch, do not immediately overwrite frontend quota with `file-service /quota` response because `auth_client` there caches quota for 60s. `auth-service` should invalidate `file-service` quota cache via internal API, while frontend updates local quota optimistically.
- 2026-06-13: Frontend must treat `auth-store.user.quotaBytes` as source of truth for plan limit and `file-store.quota.used` as source of truth for current usage. `file-service /quota` may lag briefly behind plan changes because of inter-service quota caching.
