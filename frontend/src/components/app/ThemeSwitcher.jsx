...
import { Check, Laptop, MoonStar, Palette, Settings2, SunMedium } from 'lucide-react'
...
function ThemeSwitcher({ align = 'end', compact = false, settingsMode = false }) {
  const { resolvedTheme, setTheme, theme } = useTheme()
  const Icon = settingsMode ? Settings2 : getThemeIcon(resolvedTheme, theme)
    ...
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        <Button className={cn('gap-2', compact ? 'px-3' : 'px-4')} variant="outline">
          ...
          {settingsMode ? null : <span className={cn(compact && 'hidden md:inline')}>{selectedOption.label}</span>}
        </Button>
        ...
        >
          <div className="px-3 py-2">
            <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">
              {settingsMode ? 'Settings' : 'Theme'}
            </p>
            <p className="mt-2 text-sm font-medium">
              {settingsMode ? 'Оформление интерфейса' : 'Оформление интерфейса'}
            </p>
          </div>
