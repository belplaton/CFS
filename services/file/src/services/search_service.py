...
class SearchService:
    ...
    async def search(self, user_id: UUID, query: str, limit: int = 50) -> list[ItemResponse]:
        pattern = f"%{query}%"

        files = await FileRepository.search_by_name(self.db, user_id, pattern, limit)
        folders = await FolderRepository.search_by_name(self.db, user_id, pattern, limit)
